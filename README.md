### Hi there 👋
[![Typing SVG](https://readme-typing-svg.demolab.com/?lines=Hello+I'm+Badmus+Qudus+Ayomide+|+Full+Stack+Developer+|+Founder)](https://git.io/typing-svg)
[![GitHub Streak](https://streak-stats.demolab.com/?user=BadmusQudusAyomide&cache_seconds=1)](https://git.io/streak-stats)
![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight&layout=compact)

---

## 🛠️ Tech Stack

**Languages**
[![Languages](https://skillicons.dev/icons?i=js,ts,python,java,php,dart,go,c,kotlin&perrow=9)](https://skillicons.dev)

**Frontend**
[![Frontend](https://skillicons.dev/icons?i=react,nextjs,vue,angular,flutter,tailwind,html,css&perrow=8)](https://skillicons.dev)

**Backend & APIs**
[![Backend](https://skillicons.dev/icons?i=nodejs,express,django,laravel,graphql,fastapi&perrow=6)](https://skillicons.dev)

**Mobile**
[![Mobile](https://skillicons.dev/icons?i=flutter,dart,kotlin,react&perrow=4)](https://skillicons.dev)

**Databases**
[![Databases](https://skillicons.dev/icons?i=mongodb,mysql,supabase,redis,firebase&perrow=5)](https://skillicons.dev)

**Tools & Platforms**
[![Tools](https://skillicons.dev/icons?i=git,github,docker,vscode,figma,aws,nginx,kubernetes,cloudflare,postman,linux&perrow=6)](https://skillicons.dev)

---

<h2><img src="https://emojis.slackmojis.com/emojis/images/1531849430/4246/blob-sunglasses.gif?1531849430" width="30"/> Good day 🙏🏻, I'm Badmus Qudus Ayomide <img src="https://media.giphy.com/media/12oufCB0MyZ1Go/giphy.gif" width="50"></h2>
<img align='right' src="https://media.giphy.com/media/M9gbBd9nbDrOTu1Mqx/giphy.gif" width="230">

>  Software Enginnering Student @ UNIOSUN ·(https://github.com/BadmusQudusAyomide)  
> I build products that solve real campus problems and AI tools that work in the real world.

<img src="https://media.giphy.com/media/WUlplcMpOCEmTGBtBW/giphy.gif" width="30">

[![Twitter Follow](https://img.shields.io/twitter/follow/AyomideQud49713?label=Follow)](https://twitter.com/intent/follow?screen_name=AyomideQud49713)
[![Linkedin](https://img.shields.io/badge/-qudus-blue?style=flat-square&logo=Linkedin&logoColor=white&link=https://www.linkedin.com/in/qudus-ayomide-badmus-13505a23b/)](https://www.linkedin.com/in/qudus-ayomide-badmus-13505a23b/)
[![Portfolio](https://img.shields.io/badge/Portfolio-46a2f1?style=flat-square&logo=Google-Chrome&logoColor=white)](https://badmusqudusayomide.vercel.app/)
![GitHub followers](https://img.shields.io/github/followers/BadmusQudusAyomide?label=Follow&style=social)
[![wakatime](https://wakatime.com/badge/user/018d65c8-1382-4efe-b44d-931697b3d78c.svg)](https://wakatime.com/@018d65c8-1382-4efe-b44d-931697b3d78c)

<a href="https://gitroll.io/profile/uzvMPpduKk9aMmkcminljIi08tTC3" target="_blank"><img src="https://gitroll.io/api/badges/profiles/v1/uzvMPpduKk9aMmkcminljIi08tTC3" alt="GitRoll Profile Badge"/></a>

---
```javascript
const qudus = {
    pronouns: "He/Him",
    aka: "Codestream",
    code: ["JavaScript", "TypeScript", "Python", "Java"],
    askMeAbout: ["web dev", "startups", "system design", "AI bots"],
    technologies: {
        frontEnd: {
            js: ["React", "Next.js"],
            css: ["Tailwind CSS", "Framer Motion"]
        },
        backEnd: {
            js: ["Node.js", "Express", "Fastify"],
        },
        databases: ["MongoDB", "Supabase", "MySQL"],
        misc: ["Socket.IO", "Firebase", "Resend", "Render"],
        tools: ["Git", "VSCode", "Postman", "Docker"]
    },
    currentlyBuilding: [
        "Blorbmart – a 3-sided campus marketplace (React/Next.js/Node.js)",
        "Jarvis – an AI WhatsApp receptionist for Nigerian clinics"
    ],
    architecture: ["REST APIs", "Serverless", "Progressive Web Apps", "Single Page Applications"],
    funFact: "There are two ways to write error-free programs; only the third one works"
};
```

<img src="https://media.giphy.com/media/LnQjpWaON8nhr21vNW/giphy.gif" width="60"> <em><b>I love connecting with different people</b> so if you want to say <b>hi, I'll be happy to meet you more!</b> 😊</em>

---

[![CodeTime Badge](https://img.shields.io/endpoint?style=social&color=222&url=https%3A%2F%2Fapi.codetime.dev%2Fshield%3Fid%3D25430%26project%3D%26in=0)](https://codetime.dev)

![Github Stats](https://raw.githubusercontent.com/BadmusQudusAyomide/github-stats/master/generated/overview.svg#gh-dark-mode-only)
![Github Stats](https://raw.githubusercontent.com/BadmusQudusAyomide/github-stats/master/generated/overview.svg#gh-light-mode-only)
![Github Stats](https://raw.githubusercontent.com/BadmusQudusAyomide/github-stats/master/generated/languages.svg#gh-dark-mode-only)
![Github Stats](https://raw.githubusercontent.com/BadmusQudusAyomide/github-stats/master/generated/languages.svg#gh-light-mode-only)

[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=BadmusQudusAyomide&layout=compact&theme=tokyonight&langs_count=10)](https://github.com/BadmusQudusAyomide/github-readme-stats)

📊 **This Week I Spent My Time On**

<!--START_SECTION:waka-->
<!--END_SECTION:waka-->
